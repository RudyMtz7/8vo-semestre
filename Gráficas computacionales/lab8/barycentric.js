...
function recalculateBarycentricCoords() {
    ...
    var limite_a = v3.position.clone().sub(v2.position);
    var limite_b = v1.position.clone().sub(v3.position);
    var limite_c = v2.position.clone().sub(v1.position);
    var borde_a = p.position.clone().sub(v1.position);
    var borde_b = p.position.clone().sub(v2.position);
    var borde_c = p.position.clone().sub(v3.position);
    var cruz_a = limite_a.clone().cross(borde_c.clone());
    var cruz_b = limite_b.clone().cross(borde_a.clone());
    var cruz_c = limite_c.clone().cross(borde_b.clone());
    var cruz_aux = limite_a.clone().cross(limite_b.clone());
    var resultado = cruz_aux.clone().normalize();
    var triangulo_a = resultado.dot(cruz_aux) / 2;
    var triangulo_b = resultado.dot(cruz_a) / 2;
    var triangulo_c = resultado.dot(cruz_b) / 2;
    var triangulo_d = resultado.dot(cruz_c) / 2;

    b1 = triangulo_b / triangulo_a;
    b2 = triangulo_c / triangulo_a;
    b3 = triangulo_d / triangulo_a;
